...
pub fn compute_mel_spectrogram(audio: &[f32], config: &PreprocessorConfig) -> Result<Array2<f32>> {
    // 1. Apply preemphasis
    ...
    let mel_filterbank =
        create_mel_filterbank(config.n_fft, config.features, config.sample_rate as usize);
